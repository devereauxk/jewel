...
TEMPLATE=korinna/ZJet_pp8160v2.dat
NJOBS=500
PARALLEL=5

echo "Generating 500k pp 8160 GeV events (${NJOBS} jobs x 1000, ${PARALLEL} parallel)"
echo "PTMIN=5, PTMAX=1200, WEXPO=1.2"
echo "Started: $(date)"

count=0
for (( c=1; c<=NJOBS; c++ )); do
    sed "s/xxxx/$c/g" "$TEMPLATE" > "parameters/ZJet_pp8160v2_$c.dat"
    ./jewel-2.4.0-vac "parameters/ZJet_pp8160v2_$c.dat" &
    count=$((count + 1))
...
OUTROOT="/raid5/data/kdevero/jewel_workspace/validation/jewel_pp8160v2_500k.root"

"$CONVERTDIR/ConvertHepMCToRoot" eventfiles/out_ZJet_pp8160v2_*.hepmc "$OUTROOT"
