...
TEMPLATE="korinna/ZJet_pPb_v3.dat"
MAX_PARALLEL=5

echo "Generating ${TOTAL_EVENTS} pPb 8160 GeV events with JEWEL 2.4.0-MOD + 2D hydro (KEEPRECOILS F)"
echo "PTMIN=0, PTMAX=1200, WEXPO=1.4, KEEPRECOILS=F"
echo "Started: $(date)"
...
MEDEOF
    ...
    sed -e "s/xxxx/$bin/g" -e "s/yyyy/$nevt/g" "$TEMPLATE" > "parameters/ZJet_pPb_v3_${bin}.dat"

    echo "[$((i+1))/$NBINS] Launching bin $bin ($nevt events)..."
    ./jewel-2.4.0-2D "parameters/ZJet_pPb_v3_${bin}.dat" &
...
OUTROOT="/raid5/data/kdevero/jewel_workspace/validation/jewel_pPb_v3_MOD_norecoil_500k.root"

"$CONVERTDIR/ConvertHepMCToRoot" eventfiles/out_ZJet_pPb_v3_*.hepmc "$OUTROOT"
